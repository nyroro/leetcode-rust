...
impl Solution {
    pub fn check_valid_grid(grid: Vec<Vec<i32>>) -> bool {
        ...
        let mut moves = vec![(1, 2), (1, -2), (-1, 2), (-1, -2), (2, 1), (2, -1), (-2, 1), (-2, -1)];

        fn is_valid_move(x: i32, y: i32, n: i32, visited: &Vec<Vec<bool>>) -> bool {
            x >= 0 && x < n && y >= 0 && y < n && !visited[x as usize][y as usize]
        }

        fn dfs(x: i32, y: i32, count: i32, n: i32, grid: &Vec<Vec<i32>>, visited: &mut Vec<Vec<bool>>) -> bool {
            if count == n * n {
                return true;
            }
            ...
            for (dx, dy) in moves.iter() {
                ...
                if is_valid_move(nx, ny, n, visited) && grid[nx as usize][ny as usize] == count {
                    ...
                    if dfs(nx, ny, count + 1, n, grid, visited) {
                        return true;
                    }
